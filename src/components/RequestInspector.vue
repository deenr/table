<template>
  <section class="flex h-full min-h-0 flex-col">
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-4 py-2 md:overflow-hidden">
    <article
      v-for="request in requests"
      :key="request.requestId"
      class="grid grid-cols-[1fr_2fr] items-baseline gap-x-3 gap-y-1 border-b border-border py-3 last:border-b-0"
    >
      <p class="text-[10px] font-semibold uppercase leading-5 tracking-[0.08em] text-muted-foreground">
        Status
      </p>
      <Badge
        :class="{
